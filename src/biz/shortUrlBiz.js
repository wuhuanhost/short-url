    ...
    async.series(
        [
            function(callback) {
                ...
                shortUrlDao.readSysConfig("max_index", function(err, result) {
                    if (!err) {
                        ...
                        let maxIndexLastNumber = tempMaxIndex.substr(
            ...
            function(callback) {
                ...
                shortUrlDao.addShortUrl(urlCode, url, tablename, function(
                    err,
                    result
                ) {
                    if (!err) {
            ...
            function(callback) {
                // 配置表中的最大记录值+1
                shortUrlDao.updateSysConfig("max_index", maxIndex, function(
                    err,
                    result
                ) {
                    if (!err) {
